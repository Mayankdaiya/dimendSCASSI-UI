...
const DesignRing = () => {
    ...
    const content = (
        <div className='w-1/4 min-w-80 absolute flex flex-col items-center top-1/2 left-1/2 -translate-1/2 text-center'>
            ...
            <div className='w-[70%]'><button className='btn-style2 uppercase lg:text-sm text-xs mb-2' aria-label='start setting'>Start with a setting</button></div>
            <div className="h-[2.3rem] w-[70%] group flex flex-col items-center lg:text-sm text-xs">
                <button onClick={()=>setShow(!show)} className='btn-style2 uppercase flex justify-center items-center gap-3' aria-label='start diamond'>Start with a Diamond <span className={`down-btn-icon lg:group-hover:rotate-180 ${show?'rotate-180':''}`}></span></button>
                <div className={`${show?'block':'hidden'} w-full lg:group-hover:block pt-2`}>
                    <button className='block btn-style2' aria-label='lab'>LAB</button>
                    <button className='block btn-style2 my-0.5' aria-label='natural'>NATURAL</button>
                    <button className='block btn-style2' aria-label='lab colored'>LAB COLORED</button>
                </div>
            </div>
        </div>
    );
